...
class MowerSpinRightInstructionProcessorTest {

    @InjectMocks
    private MowerSpinRightInstructionProcessor underTest;

    @Test
    void isInstructionSupported_shouldReturnTrueForSpinRight() {
        assertThat(underTest.isInstructionSupported(SPIN_RIGHT)).isTrue();
    }
    ...
    private static Stream<Arguments> getInstructionsForNotSupportedInstructionsTest() {
        return Stream.of(Arguments.of(SPIN_LEFT), Arguments.of(MOVE_FORWARD));
    }

    private static Stream<Arguments> getNewOrientationsForApplyInstructionTest() {
        return Stream.of(buildNewOrientationsArgument(NORTH, EAST),
            buildNewOrientationsArgument(EAST, SOUTH),
            buildNewOrientationsArgument(SOUTH, WEST),
            buildNewOrientationsArgument(WEST, NORTH));
    }
